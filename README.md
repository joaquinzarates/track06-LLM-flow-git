...
<details>
  ...
  <ol>
    ...
    <li><a href="#visualización-del-reporte">Visualización del reporte</a></li>
<li><a href="#capturas-de-ejecución">Capturas de ejecución</a></li>
...
- Reporte Markdown con resumen ejecutivo, tabla por impacto y descripción de cada hallazgo con fragmento de código y recomendaciones
- Clasificación por categorías: seguridad (OWASP Top 10 2025), errores
  potenciales, rendimiento y estilo
- Manejo de errores de API: reintentos con backoff exponencial ante respuestas 429 y formato inesperado
- Variables sensibles gestionadas con `python-dotenv`, nunca harcodeadas ( en el código)
- Repositorio de prueba con 5 defectos intencionales verificados con el LLM y con SonarQube (extensión VS Code)
...
1. Edita `.env` y agrega tu API key:

```sh
GEMINI_API_KEY='tu_api_key_aqui'
```

Obtener API key gratuita en [Google AI Studio](https://aistudio.google.com)
...
El reporte generado es un archivo Markdown. Para visualizarlo con dicho formato en VS Code se recomienda instalar la extensión **Markdown Preview Enhanced**:

1. Abre VS Code → Extensions (`Ctrl+Shift+X`)
2. Busca `Markdown Preview Enhanced`
3. Instala la extensión de Yiyi Wang
4. Abre el archivo `.md` generado y presiona `Ctrl+Shift+V`

Enlace directo: [Markdown Preview Enhanced](https://marketplace.visualstudio.com/items?itemName=shd101wyy.markdown-preview-enhanced)

---

## Capturas de ejecución

### 1. Generación del diff

![Generación del diff](capturas/01-generacion-diff.png)
*Ejecución de `git diff --cached` dentro del repositorio de prueba para generar el archivo `.diff`*

### 2. Archivos generados — diff

![Archivos en VS Code](capturas/02-vscode-archivos-diff.png)
*Vista en VS Code confirmando la creación del archivo `cambios.diff`*

### 3. Ejecución de la herramienta

![Ejecución](capturas/03-ejecucion.png)
*Llamada a `main.py` con el modo `--archivo`, análisis con Gemini 2.5 Flash y confirmación del reporte generado*

### 4. Archivos generados — reporte

![Archivos reporte en VS Code](capturas/04-vscode-archivos-reporte.png)
*Vista en VS Code confirmando la creación del archivo `reporte_ejemplo.md`*

### 5. Reporte renderizado

![Reporte con preview](capturas/05-reporte-codigo-preview.png)
*Vista dividida en VS Code: código Markdown a la izquierda y preview renderizado con Markdown Preview Enhanced a la derecha*
...
- **OWASP Top 10 2025:** Se utilizó la versión más reciente del estándar, presentada el año pasado, mostrando cambios en el ranking o bien el orden de aparición de las vulnerabilidades.
